...
class AdjacencyListWeightedVertex<T>
{
    public T? Value;
    public List<int> ConnectionList = new List<int>();
    public List<int> WeightList = new List<int>();

    public AdjacencyListWeightedVertex(T? t)
    {
        Value = t;
    }

    public bool IsAdjacentWith(int vertexNumber)
    {
        return ConnectionList.Contains(vertexNumber);
    }
    ...
    public void AddAdjacent(int vertexNumber)
    {
        ConnectionList.Add(vertexNumber);
        WeightList.Add(0);
    }
    
    public void RemoveAdjacent(int vertexNumber)
    {
        int index = ConnectionList.IndexOf(vertexNumber);
        if (index >= 0)
        {
    ...
    public void ReplaceAdjacent(int target, int replace)
    {
        if (ConnectionList.Contains(target))
        {
            ConnectionList.Remove(target);
            ConnectionList.Add(replace);
        }
    }

    public int GetWeight(int adjacentVertexNumber)
    {
        int index = ConnectionList.IndexOf(adjacentVertexNumber);
        return WeightList[index];
    }

    public void SetWeight(int adjacentVertexNumber, int weight)
    {
        int index = ConnectionList.IndexOf(adjacentVertexNumber);
...
public class AdjacencyListWeightedDirectedGraph<T> : IWeightedGraph<T>, IDirectedGraph<T>
{
    ...
    class Enumerator : IEnumerator<T>
    {
        ...
        public Enumerator(AdjacencyListWeightedDirectedGraph<T> graph)
        {
