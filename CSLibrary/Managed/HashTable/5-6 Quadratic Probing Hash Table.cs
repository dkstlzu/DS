...
public class QuadraticProbingHashTable<T> : IHashTable<T> where T : IEquatable<T>
{
    ...
    public QuadraticProbingHashTable(int minimumCapacity)
    {
    ...
    private HashTableNode<T>? Find(int probingStartIndex, int key, out int lastProbedIndex)
    {
        ...
        for (int i = 0; i < _elements.Length; i++, probingIndex = (probingStartIndex + i*i) % _elements.Length)
        {
    ...
    private bool TrySet(int key, T? t)
    {
        ...
        var targetNode = Find(realKey, key, out int lastProbedIndex);
    ...
    public bool Contains(int key)
    {
        ...
        return Find(realKey, key, out _) != null;
    }
    ...
    {
        private QuadraticProbingHashTable<T> _hashTable;
        private int _index;
        private T? _current;
        
        public Enumerator(QuadraticProbingHashTable<T> hashTable)
        {
